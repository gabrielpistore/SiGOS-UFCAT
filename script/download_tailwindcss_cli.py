...
if MACHINE in ["amd64", "x86_64"]:
    ...
    raise Exception(f"{colorama.Fore.RED}Unsupported architecture: {MACHINE}{colorama.Style.RESET_ALL}")
...
print(f"{colorama.Fore.CYAN}Downloading TailwindCSS CLI from {tailwindcss_url}...")
response = requests.get(tailwindcss_url)

if response.status_code == 200:
    with open(output_file, "wb") as file:
        file.write(response.content)
    print(f"{colorama.Fore.GREEN}Downloaded TailwindCSS CLI to {output_file}{colorama.Style.RESET_ALL}")
else:
    raise Exception(
...
print(f"{colorama.Fore.GREEN}TailwindCSS CLI is ready to use!{colorama.Style.RESET_ALL}")
